...
**Smoke、P0、P1 的核心验收条件已经满足；P2 的代码、真实空间叠加、五族能力边界、训练后交互页和两分钟视频已经满足；独立 Final 仓库与阶段导航也已建立。正式提交前只剩两项外部决定：评审访问前把 Final 仓库设为公开或添加评审协作者，以及确认“工具 PR”最终应投向课程收集仓库还是 Tencent/YOLO-Master。**
...
1. 评审访问前，将当前私有的 `e3-routing-final-report` 设为公开，或把评审账号加入协作者。
2. 合并 P2 的训练后演示 PR，使 P2 主分支导航和 MP4 生效。
3. 在答辩材料中统一使用总 manifest 锁定的这组数字。
